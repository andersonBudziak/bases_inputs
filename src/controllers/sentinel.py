...
class Sentinel2Processor:
    ...
    def get_filtered_df(self):
        ...
        df = pd.DataFrame(data, columns=['id', 'date', 'ndvi'])

        df_filtered = df.copy()

        df_filtered['ndvi'] = df_filtered[['ndvi']].applymap(
            lambda x: None if x < 0 else x)
        df_filtered['ndvi'] = df_filtered[['ndvi']].replace(-9999, None)
        df_filtered = df_filtered.dropna(subset='ndvi')
            # Creating DataFrame and processing

        df_filtered['satellite'] = 'Sentinel2'
        df_filtered.dropna(inplace=True)

        # Handling date and sorting
        df_filtered.sort_values('date', inplace=True)
        df_filtered = df_filtered.drop_duplicates(subset='date').reset_index(drop=True)


        return df_filtered[['date', 'id', 'ndvi','satellite']]#
